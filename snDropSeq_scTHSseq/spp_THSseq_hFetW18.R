...
## pooled data. $tags will represent Tn5 cut positions on each chromosome (sign carries the strand information), $cell will give the id (integer) of a cell giving the read; both will be sorted in the chromosome coordinate order
pdata <- mclapply(chrl,function(chr) list(tags=unlist(lapply(1:length(bamdata),function(i) { na.omit(bamdata[[i]][[chr]])})), cells=unlist(lapply(1:length(bamdata),function(i) { rep(i,length(na.omit(bamdata[[i]][[chr]])))}))), mc.cores=n.cores)
# sort
pdata <- lapply(pdata,function(d) { co <- order(abs(d$tags),decreasing=F); return(list(tags=d$tags[co],cells=d$cells[co]))})
#head(pdata)
...
df <- data.frame(seqnames=seqnames(gr),
                   ...
                   strands=strand(gr))
head(df)
write.table(df, file=paste0("results/hFet_peaks_spp_WITHrepeats_bandwidth", bandwidth, "_step", step, "_thr", thr, "_span", span, "_fdr", fdr, ".bed"), quote=F, sep="\t", row.names=F, col.names=F)
save(peaks.spp, file=paste0("results/hFet_peaks_spp_WITHrepeats_bandwidth", bandwidth, "_step", step, "_thr", thr, "_span", span, "_fdr", fdr, ".RData"))   

require(BiocParallel)
lapply(rev(1:length(curbams)), function(i) {
    cb <- paste0(curbams[[i]], ".bam")
    cbn <- names(curbams)[i]
    print(cbn)
    cov <- do.call(cbind, bplapply(cb, function(fname) {
        pdata.cell <- read.bam.tags(fname)$tags
    ...
    save(cov, file=paste0("results/", cbn, "_cov_peaks_spp_WITHrepeats_bandwidth", bandwidth, "_step", step, "_thr", thr, "_span", span, "_fdr", fdr, ".RData"))
    rm(cov);
    gc();
})
